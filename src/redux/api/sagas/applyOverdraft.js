...
export const applyOverdraft = function* () {
    ...
    yield takeEvery(actions.APPLY_OVERDRAFT, function*(action) {
        ...
        /* START LOADING */
        yield put({ type: 'GLOBAL_LOADING_START', payload: { msg: 'Solicitando crédito'}})

        /* SEND REQUEST TO SERVER */
        console.log(' -- saga:applyOverdraft:: account:', action.payload.business_name, ' -- signature:', action.payload.signature);
        ...
        if(typeof requestEndorse.err !== 'undefined' || typeof requestEndorse.data.error !== 'undefined') {
            console.log(' redux/api/saga/applyOverdraft::applyOverdraft() ===== #1' ,JSON.stringify(requestEndorse));
        ...
        /* HANDLE ERROR RESOPONSE */
        if(typeof signData.data.error !== 'undefined' || typeof signData.err !== 'undefined') {
            console.log(' ===== #3' ,JSON.stringify(signData));
            yield put({ type: actions.APPLY_OVERDRAFT_FAILD, payload: { err: signData.err, error: signData.data.error }})
            //REMOVE LOADING AND DISPLAY ERROR
            yield put({ type: 'GLOBAL_LOADING_END'})
            yield put({ type: 'GLOBAL_MSG', payload: {
                msgType: 'error',
        ...
        /* HANDLE SUCCESS RESOPONSE */
        console.log('===========> pushAndSignTX()::res ==> ', JSON.stringify(signData));
        yield put({ type: actions.APPLY_OVERDRAFT_SUCCESS, payload: signData.data })
        // RELOAD PROFILE DATA
