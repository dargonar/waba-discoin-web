  ...
  yield takeEvery(actions.LOGIN_REQUEST, function*(action) {
    ...
    if (typeof secretRes.ex !== "undefined" || typeof secretRes.data.error !== "undefined") {
      ...
      try {
        ...
        console.log("[redux/auth/saga]-- auth/saga loginRequest err:", JSON.stringify(data.error));
        if (data && data.login === true) {
          ...
          yield put({
            type: actions.LOGIN_ERROR,
            payload: {
              err: typeof data.error !== "undefined" ? data.error.message : undefined,
              error: data.error
            }
          });
          yield put({ type: actionsUI.GLOBAL_LOADING_END });
