...
export function* loginRequest() {
  ...
  yield takeEvery(actions.LOGIN_REQUEST, function*(action) {
    ...
    yield put({
      ...
      payload: { msg: "Iniciando sesión " + account_name }
    });
    ...
    if (typeof from_storage_data !== "undefined") {
      ...
      console.log(
        "[redux/auth/saga]---- from_storage_data -- account_name:",
        account_name,
        " -- account:",
      ...
      if (just_registered_data != null) {
        ...
        */
        console.log(
          "[redux/auth/saga]---- just_registered_data :",
          JSON.stringify(just_registered_data)
        );
        ...
        console.log(
          "[redux/auth/saga]---- just_registered_data -- account_name:",
          account_name,
          " -- account:",
          JSON.stringify(account)
        );
    ...
    console.log(
      "[redux/auth/saga]-- auth/saga loginRequest:account: ",
      JSON.stringify(account)
    );
    console.log(
      "[redux/auth/saga]-- auth/saga loginRequest:account_name: ",
      account_name
    );
    //METAHACK:
    ...
    if (
      typeof secretRes.ex !== "undefined" ||
      typeof secretRes.data.error !== "undefined"
    ) {
      console.log(
        "[redux/auth/saga]-- auth/saga loginRequest ERROR",
        secretRes
      );
      yield put({
        type: actions.LOGIN_ERROR,
...
export function* register() {
  yield takeEvery(actions.REGISTER, function*(action) {
    yield put({
      type: actionsUI.GLOBAL_LOADING_START,
      payload: { msg: "Registrando comercio" }
    });
    ...
    if (err || typeof data.error !== "undefined") {
      console.log(
        " [auth/saga/register]::register() #1 ---- ERROR ",
        JSON.stringify(err),
        JSON.stringify(data)
      );
