...
class ListStores extends Component {
  ...
  getDummyTx() {
    return JSON.parse(
      '{    "expiration": "20180426T191755",    "operations": [      [        7,        {          "account_to_list": "1.2.22",          "authorizing_account": "1.2.18",          "fee": {            "amount": 300000,            "asset_id": "1.3.0"          },          "new_listing": 0        }      ],      [        0,        {          "amount": {            "amount": 25000,            "asset_id": "1.3.8"          },          "fee": {            "amount": 2096679,            "asset_id": "1.3.0"          },          "from": "1.2.18",          "memo": {            "message": "7e69653a3138303432363a312e322e32323a3235303030"          },          "to": "1.2.22"        }      ],      [        7,        {          "account_to_list": "1.2.22",          "authorizing_account": "1.2.18",          "fee": {            "amount": 300000,            "asset_id": "1.3.0"          },          "new_listing": 2        }      ]    ],    "ref_block_num": 307149,    "ref_block_prefix": 1342226333}'
    );
  ...
  sign(tx) {
    ...
    var sig = Signature.signBuffer(
      Buffer.concat([
        new Buffer(chain_id, "hex"),
        new Buffer(JSON.stringify(tx), "hex")
      ]),
      private_key
    );
    // ,public_key
  ...
  sign2(tx) {
    ...
    console.log(
      _signature ==
        "1f0694579f042cad38f297e70aed495bfdae16c90f5efc5e5a59d3d84a797527ab4f76f1ad2cf528c74398ec0a5c06e379bd2c2337d3aa3e57e709c02772a05e85"
    );
    return _signature;
  }
  ...
  createOverdraft(amount) {
    ...
    console.log(
      " Business Account ID=== > ",
      this.state.businessSelected.account_id
    );
    // 1 get tx by query
    ...
    fetch(url, {
      ...
      .then(
        responseJson => {
          ...
          console.log(
            "createOverdraft():: about to push tx ==> ",
    ...
    return (
      <div style={{ width: "100%" }}>
        {this.props.business.map(store => (
          <StoreCard {...store} key={store.id} overdraft={this.showOverdraft} />
        ))}
      </div>
    );
  }
