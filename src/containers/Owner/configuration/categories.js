...
class Categories extends Component {
  ...
  getCell(col, row) {
    return this.state.data.categories
      ...
      .reduce((prev, act) => (typeof act !== "undefined" ? act : prev), {
        empty: true
      });
  }
  ...
  onColumnChange(col) {
    return value => {
      this.setState({
        data: {
          ...this.state.data,
          cols: this.state.data.cols.map(
            column =>
              column.col === col.col
                ? Object.assign({}, col, { refund_rate: Number(value) || 0 })
                : column
          )
        }
      });
  ...
  onRowChange(row) {
    return value => {
      this.setState({
        data: {
          ...this.state.data,
          rows: this.state.data.rows.map(
            row_ =>
              row_.row === row.row
                ? Object.assign({}, row_, { description: value || "" })
                : row_
          )
        }
      });
  ...
  render() {
    ...
    if (
      ...
      this.props.configuration.categories !== null
    ) {
    ...
    const renderTable = () => {
      return this.state.data !== false ? (
        <LayoutContent>
          <TableWrapper
            pagination={false}
            columns={getColumns(this.state.data.cols)}
